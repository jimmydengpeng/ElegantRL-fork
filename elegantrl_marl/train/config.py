...
def get_gym_env_args(env, if_print) -> dict:  # [ElegantRL.2021.12.12]
    ...
    if {'unwrapped', 'observation_space', 'action_space', 'spec'}.issubset(dir(env)):  # isinstance(env, gym.Env):
        ...
        if_discrete = isinstance(env.action_space, env.action_space.Discrete)
         
        if if_discrete:  # make sure it is discrete action space
            action_dim = env.action_space.n
        elif isinstance(env.action_space, env.action_space.Box): # make sure it is continuous action space
            action_dim = env.action_space.shape[0]
            if not any(env.action_space.high - 1):
                debug_print('env.action_space.high: ', args=env.action_space.high, level=LogLevel.WARNING, inline=True)
            if not any(env.action_space.low - 1):
                debug_print('env.action_space.low', args=env.action_space.low, level=LogLevel.WARNING)
        else:
...
'''for type checking'''
DEFAULT_ARGS: Arguments = None # type: ignore