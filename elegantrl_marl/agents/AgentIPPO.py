...
from elegantrl_marl.agents.net import ActorPPO, ActorDiscretePPO, CriticPPO, SharePPO
from elegantrl_marl.train.config import \
    Arguments, DEFAULT_ARGS
from elegantrl_marl.train.replay_buffer import ReplayBufferList
...
class AgentPPO(AgentBase):
    ...
    def explore_one_env(self, env, horizon_len) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
        ...
        ary_state_dict = self.last_states_dict

        get_action = self.act.get_action_logprob
        convert = self.act.convert_action_for_env
        '''only collect `horizon_len` length of tajectories'''
        for i in range(horizon_len): 
            state = torch.as_tensor(ary_state_dict, dtype=torch.float32, device=self.device) # TenosrSize: [state_dim]
            action, logprob = [t.squeeze() for t in get_action(state.unsqueeze(0))] # 在指定位置插入维度1, 变成 Size([1, state_dim])

            ary_action = convert(action).detach().cpu().numpy()
            ary_state_dict, reward, done, _ = env.step(ary_action) # => only put action of <numpy.ndarray> to env
            if done:
                ary_state_dict = env.reset()
        ...
        self.last_states_dict[0] = ary_state_dict # 总是把最后一次与环境交互得到的（还没有返回的）state 作为 self.states[0]
    ...
    def get_advantages(self, rewards: Tensor, undones: Tensor, values: Tensor) -> Tensor:
        ...
        if self.last_states_dict:
            next_state = torch.tensor(np.array(self.last_states_dict), dtype=torch.float32).to(self.device)
