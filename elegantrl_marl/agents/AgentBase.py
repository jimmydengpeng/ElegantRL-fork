...
from elegantrl_marl.train.replay_buffer import ReplayBuffer
...
class AgentBase:
    def __init__(self, net_dim: int, state_dim: int, action_dim: int, gpu_id: int = 0, args = None):
        ...
        self.last_states_dict: Dict[str, np.ndarray] = {}
    ...
    def explore_one_env(self, env, horizon_len: int) -> list:
        ...
        traj_list = []
        last_dones = [0, ]
        state = self.last_states_dict #FIXME
    ...
    def explore_vec_env(self, env, target_step: int) -> list: # FIXME args target_step
        ...
        states = self.last_states_dict if self.if_use_old_traj else env.reset()
